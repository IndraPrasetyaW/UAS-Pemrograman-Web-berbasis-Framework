    ...
    public function actionCreate()
{
    ...
    if ($request->isAjax) {
        ...
        if ($request->isGet) {
            return [
                ...
                'footer' => Html::button('Close', ['class' => 'btn btn-default pull-left', 'data-bs-dismiss' => "modal"]) .
                    ...
                    $uploadedFile = \yii\web\UploadedFile::getInstance($model, 'foto');
                    if ($uploadedFile!== null) {
                        $model->foto = $uploadedFile->baseName. '.'. $uploadedFile->extension;
                        $uploadedFile->saveAs(Yii::getAlias('@uploads'). '/'. $model->foto);
                    }
                ...
                return [
                    ...
                    'title' => "Create new mahasiswa",
                    'content' => '<span class="text-success">Create mahasiswa success</span>',
                    'footer' => Html::button('Close', ['class' => 'btn btn-default pull-left', 'data-bs-dismiss' => "modal"]) .
        ...
        if ($model->load($request->post())) {
            ...
            if ($model->validate() && $model->save()) {
                $model->foto->saveAs(Yii::getAlias('@uploads') . '/' . $model->foto->baseName . '.' . $model->foto->extension);
                return $this->redirect(['view', 'id' => $model->id]);
            } else {
